    ...
    public function resolveConflict($data)
    {
        ...
        for ($i = 1; $i <= 10; $i++) {
            if ($data['t' . $i] > 0) {
                ...
                if (in_array($unit, $unitsbytype['chief'])) {
                    $UnitChief = $i;
                }
        ...
        if ($data['type'] == 1) {
            ...
            for ($i = 1; $i <= 38; $i++) {
                if ($DefenderFields['f' . $i] > 0) {
                    $DefenderFieldsArray[] = $i;
                }
            ...
            if ($catapults > 0 && !$IsOasis) {
                ...
                for ($i = 1; $i <= 2; $i++) {
                    ...
                    if ($DefenderData['capital'] != 1 && $data['ctar' . $i] <= 18 || in_array($data['ctar' . $i], $TrapperArray)) {
                        $BuildingLevelMax[$i] = 10;
                    }
                    if (in_array($data['ctar' . $i], $ResourceImprovementArray)) {
                        $BuildingLevelMax[$i] = 5;
                    }
                ...
                for ($i = 1; $i = ($data['ctar1'] == $data['ctar2'] ? 1 : 2); $i++) {
                    ...
                    if ($data['ctar1'] != $data['ctar2'] && $i == 1) {
                        $CatapultsFiring /= 2;
                    }
                    if ($CatapultsFiring >= $RequiredCatapults[$i]) {
                        if ($DefenderField['f' . $data['ctar' . $i]] == $FieldPalRes) {
                            $DestroyedPalRes = True;
                        }
                        if ($data['ctar' . $i] >= 19) {
                            $database->setVillageLevel($data['to'], 'f' . $data['ctar' . $i] . 't', 0);
                        }
