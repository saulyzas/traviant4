...
<head>
    <style type="text/css" media="screen" id="jScroll.css">
        ...
        .jScroll .jScroll-scroll-up {
            position: absolute;
            width: 100%;
        }

        .jScroll .jScroll-scroll-slider {
            position: absolute;
            width: 100%;
        }

        .jScroll .jScroll-scroll-down {
            position: absolute;
            width: 100%;
        }
        ...
        .jScroll .jScroll-scroll-slider .top {
            width: 100%;
        }

        .jScroll .jScroll-scroll-slider .center {
            width: 100%;
        }

        .jScroll .jScroll-scroll-slider .bottom {
            width: 100%;
        }
        ...
        .jScroll .jScroll-element-down {
            background: transparent url(crypt.js?<?php echo time(); ?>images/down.gif) no-repeat center bottom;
            height: 6px;
        }

        .jScroll .jScroll-element-down:hover {
            background-image: url(crypt.js?<?php echo time(); ?>images/down-hover.gif);
        }

        .jScroll .jScroll-element-up {
            background: transparent url(crypt.js?<?php echo time(); ?>images/up.gif) no-repeat center top;
            height: 6px;
        }

        .jScroll .jScroll-element-up:hover {
            background-image: url(crypt.js?<?php echo time(); ?>images/up-hover.gif);
        }

        .jScroll .jScroll-element-between {
            margin-right: 4px;
            width: 7px;
            opacity: 0;
        ...
        .jScroll .jScroll-element-between:hover {
            opacity: 0.25;
            filter: alpha(opacity=25);
        }

        .jScroll .jScroll-element-between.clicked {
            opacity: 0.5;
            filter: alpha(opacity=50);
        }

        .jScroll .jScroll-element-slider .top {
            background: transparent url(crypt.js?<?php echo time(); ?>images/slider-top.gif) no-repeat center top;
            height: 1px;
        }

        .jScroll .jScroll-element-slider:hover .top {
            background-image: url(crypt.js?<?php echo time(); ?>images/slider-top-hover.gif);
        }

        .jScroll .jScroll-element-slider .center {
            background: transparent url(crypt.js?<?php echo time(); ?>images/slider-center.gif) repeat-y center top;
        }

        .jScroll .jScroll-element-slider:hover .center {
            background-image: url(crypt.js?<?php echo time(); ?>images/slider-center-hover.gif);
        }

        .jScroll .jScroll-element-slider .bottom {
            background: transparent url(crypt.js?<?php echo time(); ?>images/slider-bottom.gif) no-repeat center bottom;
            height: 1px;
        }

        .jScroll .jScroll-element-slider:hover .bottom {
            background-image: url(crypt.js?<?php echo time(); ?>images/slider-bottom-hover.gif);
        }
    </style>
        ...
        <div class="bodyWrapper">
                ...
                <div id="mtop">
                    ...
                    <ul id="navigation">
                        ...
                        if (count($database->getMessage($session->uid, 7)) >= 1000) {
                            ...
                            $unmsg = count($database->getMessage($session->uid, 7));
                        }

                        if (count($database->getMessage($session->uid, 8)) >= 1000) {
                            $unnotice = "+1000";
                        } else {
                            $unnotice = count($database->getMessage($session->uid, 8));
                        }
                        ?>
                        <li id="n5" class="reports">
                            <a href="berichte.php" accesskey="5" title="<?php echo HEADER_NOTICES; ?><?php if ($message->nunread) {
                                                                                                            echo ' (' . count($database->getMessage($session->uid, 8)) . ')';
                                                                                                        } ?>"></a>
                            <?php
                            if ($message->nunread) {
			...
			<div class=\"bubble-content\">" . $unnotice . "</div></div>";
                        ...
                        <li id="n6" class="messages">
                            <a href="nachrichten.php" accesskey="6" title="<?php echo HEADER_MESSAGES; ?><?php if ($message->unread) {
                                                                                                                echo ' (' . count($database->getMessage($session->uid, 7)) . ')';
                                                                                                            } ?>"></a>
                            <?php
                            if ($message->unread) {
            ...
            <div id="mid">
                ...
                </div><?php include("Templates/sideinfo.php"); ?><div class="clear"></div>
